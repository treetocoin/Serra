-- Fix sensor data ingestion to work with new sensor configuration system
-- This migration updates insert_sensor_readings to support both:
-- 1. Port-based configuration (new system with device_sensor_configs)
-- 2. Auto-discovery (legacy system for backward compatibility)
...
BEGIN
  ...
  LOOP
    ...
    v_port_id := reading_item->>'port_id';
    ...
    -- NEW FLOW: If port_id is provided, use sensor configuration
    IF v_port_id IS NOT NULL THEN
      -- Get configured sensor type from device_sensor_configs
      ...
      IF v_configured_sensor_type IS NULL THEN
        RAISE EXCEPTION 'No active sensor configuration found for device % port %',
          v_composite_device_id, v_port_id;
      END IF;

      -- Use configured sensor type
