...
export function TemperatureChart({ deviceId }: Props) {
  const { data: readings, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .in('reading_sensor_type', ['dht_sopra_temp', 'dht_sotto_temp'])
  ...
  // Hide chart if no data (FR-006, FR-007)
  if (!readings || readings.length === 0) {
    return null;
  }

  // Transform readings to chart format (pivot sensor_type to columns)
...
function transformToChartFormat(readings: SensorReading[]) {
  // Group readings by timestamp, pivoting reading_sensor_type to columns
